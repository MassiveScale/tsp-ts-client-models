...
- **Pluggable HTTP client: custom transport, middleware, and lifecycle hooks.** The generated `HttpClient` base (`client/ApiClient.ts`) now exposes three extension points on `ClientConfig`, so existing auth and error-handling code can be reused with a generated client instead of being reimplemented around it:
  ...
  - **`middleware?: HttpMiddleware[]`** plus **`HttpClient.useMiddleware(middleware)`** — an onion-style pipeline, `(request, next) => Promise<Response>`, with the first entry as the outermost layer and the transport at the center. Layers can short-circuit (never call `next`), replay (call `next` twice — e.g. refresh a token on 401 and retry), or wrap the call in `try`/`catch`. `useMiddleware()` appends inside `config.middleware` and returns the client for chaining. It is deliberately not called `use`, so that an API operation named `use` keeps its own method name.
...
- **Generated modules exporting the same name no longer break the package barrel.** `index.ts` star-exports every generated module, so any two of them exporting the same name produced an ambiguous re-export and the package failed to compile with TS2308. Three variants existed: a TypeSpec type named after a client infrastructure export (`RequestContext`, `ClientConfig`, `ApiError`, `RetryConfig`, `HttpMiddleware`, …); a type named after a generated endpoint object or client class (`WidgetsEndpoints`, `WidgetsClient`); and an interface whose generated client class collides with the infrastructure (`interface RxHttp` → `RxHttpClient`, also exported by `ApiClientRx.ts`). The barrel now resolves all of them in priority order — declared TypeSpec types, then per-interface generated modules, then the static infrastructure — re-exporting the loser explicitly with a `Client` prefix (`RequestContext` → `ClientRequestContext`, numbered if taken) and reporting a `generated-export-name-collision` **warning**. Declared types always keep the plain name. Type-only exports go through `export type`, so the emitted JavaScript never references a compile-time-only binding. When nothing collides the barrel is unchanged — a plain list of `export *` lines. **Template authors:** `IndexView` gains an optional `namedExports` field; an `index` template override that ignores it will emit a broken barrel in the collision case. See the [template view models](README.md#template-view-models).
- **A model used directly as a response or body no longer clashes with the client's own imports.** A generated client module imports `HttpClient` and `RequestOptions` (plus `Observable`/`RxHttpClient` for the Observable flavor) and its `*Endpoints` object, then imports the models it references alongside them. A model named after any of those produced two bindings of the same name in one module — TS2300, duplicate identifier — even though the package barrel already resolved the clash. The client's _infrastructure_ imports are now aliased instead (`import { HttpClient as ClientHttpClient }`), so the model keeps the plain name in the generated method signatures. **Template authors:** `ClientView` gains `baseClassName`, `rxBaseClassName`, `requestOptionsName`, `observableName`, and `endpointsLocalName` — a `client`/`clientObservable` override that hardcodes the imported names will not compile in this case.
- **Generated client module names are now allocated from one shared pool, so no client is silently overwritten.** Client class (and file) names were formed directly from the interface name, with no check that the path was free, so a later write clobbered an earlier one and a client vanished from the package with no diagnostic at all. Three collisions did this: `interface Api` → `client/ApiClient.ts`, the same path as the static infrastructure; `interface Foo` (Observable flavor) and `interface FooObservable` (Promise flavor) both → `client/FooObservableClient.ts`; and `model WidgetsClient` alongside `interface Widgets`, where the client both imported and declared `WidgetsClient` (TS2440). Promise and Observable names are now allocated from a single set seeded with the infrastructure module names and every type `models.ts` exports; a client that cannot have its preferred name takes a numeric suffix and reports a `client-module-name-collision` **warning**. The infrastructure path cannot move and a declared type keeps its own name, so the generated client is what yields. `models.ts` is now rendered before the client loop so its exported names are known during allocation — the type-collection passes already complete before either, so nothing is discovered later.
- **`@head` operations no longer produce a package that fails to compile.** The generated client rendered `this.httpHead<void>(…)` for every `@head` operation, but the base `httpHead` helper declared no type parameter — so any spec with a `@head` operation failed with TS2558 ("Expected 0 type arguments, but got 1"). `httpHead` and `httpHead$` are now generic with a `void` default, matching the other verb helpers. This predates the middleware work and was caught by the new end-to-end compile tests.
- **Operation names that collide with an inherited client member are now renamed instead of breaking the build.** A generated `*Client` extends `HttpClient`, so an operation named `request`, `config`, `buildUrl`, `useMiddleware`, or one of the `httpGet`/`httpPost`/… helpers produced a method that shadowed the inherited member with an incompatible signature — and the generated package failed to type-check. Such methods now get an `Operation` suffix (`request` → `requestOperation`, numbered further if a sibling operation already claims that name), accompanied by a new `reserved-client-method-name` **warning** that names the substitution. The reserved set is resolved **per base class**: `RxHttpClient`'s own members (`observe` and the `$` verb helpers) are reserved only when an Observable client is actually emitted, so a `client-style: promise` build keeps `client.observe()` and warns about nothing. Under `client-style: both` the larger set applies to both generated clients, so the two flavors stay method-for-method interchangeable. Only the client method is renamed: the `*Endpoints` object is a plain `as const` that inherits nothing, so its keys — and the endpoint call inside the renamed method — keep the original operation name. The warning is reported once per operation even when both client flavors are emitted, and not at all under `generate-http-client: false`.
- **Middleware registration renamed `use` → `useMiddleware`.** `use` is a plausible API operation name, and reserving it would have cost users a method name for the sake of an SDK concern. `HttpClient.useMiddleware()` is far less likely to collide, and `use` is consequently _not_ on the reserved list — an operation named `use` generates `client.use()` as intended. This only affects code written against the unreleased `use()` API added earlier in this same release.
- **An unserializable request body now reaches `onError`.** `JSON.stringify` on the request body ran before the transport's outer `try`, so a circular reference or a `BigInt` rejected the call without ever invoking `onError` — contradicting its documented role as the terminal handler for every failure. Body serialization, URL construction, and header assembly all now happen inside the error-handled block. The error context still carries a usable `url` even when building the full URL with its query string is what failed.
- **Malformed `Retry-After` headers no longer produce bogus retry delays.** The header was parsed with `parseFloat`, which accepts a numeric _prefix_ and passes through negative and non-finite values — so `"2seconds"` became a 2-second delay, `"-1"` a negative one, and `"Infinity"` a retry that would never fire, hanging the call indefinitely. The complete value must now match a non-negative `delta-seconds` number (integer or decimal, surrounding whitespace tolerated) **that also lands within the maximum delay a timer can represent** (2^31−1 ms, about 24.8 days). Two range bugs hid behind the earlier check: a seconds value can be finite and overflow to `Infinity` once multiplied by 1000, and a value that is finite even in milliseconds (`1e308`) still exceeds what `setTimeout` accepts — Node wraps such a delay to roughly 1ms and browsers clamp it, so a "retry in a millennium" header caused an _immediate_ retry. Anything outside the usable range — an HTTP-date, a trailing unit, `"1e3"`, `NaN`, an overflowing or oversized digit string — yields `undefined`, and the client falls back to its own exponential backoff.
- **`RateLimitError` and `ServiceUnavailableError` are now actually thrown.** Both classes have been exported and documented since the client was introduced, but the transport only ever constructed a plain `ApiError` — so `catch (e) { if (e instanceof RateLimitError) … }` never matched. A 429 now throws a `RateLimitError` (with `retryAfterMs` parsed from the `Retry-After` header, or `undefined` when the header is absent or unparseable) and a 503 throws a `ServiceUnavailableError`. Both carry the parsed error body. Since both extend `ApiError`, existing `instanceof ApiError` and `err.status` checks are unaffected.
...
- **Generated packages are now type-checked end to end by the test suite.** A new `test/generated-package-compiles.test.ts` writes emitted output to a temp directory and runs `tsc` over it using the package's own emitted `tsconfig.json`, covering a plain package, every infrastructure name collision, and operations named after inherited client members. Content assertions only find what a test author thought to look for; this found the `@head`/TS2558 bug above on its first run. A minimal structural `rxjs` stub keeps the Observable flavor genuinely checked without installing the optional peer dependency.
