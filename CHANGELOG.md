...
- **A model or enum named after a client infrastructure export no longer breaks the generated package.** `index.ts` star-exports both `models.ts` and `client/ApiClient.ts`, so a TypeSpec type named `RequestContext`, `ClientConfig`, `ApiError`, `RetryConfig`, `HttpMiddleware` — or any other `ApiClient.ts`/`ApiClientRx.ts` export — produced an ambiguous re-export and the package failed to compile with TS2308. The barrel now detects this and re-exports the affected infrastructure module by explicit name instead of with a star, aliasing the colliding name with a `Client` prefix (`RequestContext` → `ClientRequestContext`, numbered if that too is taken) and reporting a new `client-infrastructure-name-collision` **warning**. The user's own type keeps the plain name. Type-only exports are re-exported via `export type`, so the emitted JavaScript never references a compile-time-only binding. When nothing collides the barrel is unchanged — a plain list of `export *` lines. This widened with the new `FetchFunction`/`HttpNext`/`HttpMiddleware`/`RequestContext`/`RequestHook`/`ResponseHook`/`ErrorHook` exports above, but affected `ClientConfig`, `RetryConfig`, `RequestOptions`, and the error classes already. **Template authors:** `IndexView` gains an optional `namedExports` field; an `index` template override that ignores it will emit a broken barrel in the collision case. See the [template view models](README.md#template-view-models).
