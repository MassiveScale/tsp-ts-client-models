...
- **An interface whose generated client would overwrite the infrastructure file is now renamed.** `interface Api` emitted `client/ApiClient.ts`, the same path as the static base infrastructure, which was written afterwards and silently replaced it — the generated client disappeared from the package with no diagnostic at all. The infrastructure path cannot move (every client imports `./ApiClient.js`), so the generated client is emitted as `ApiClient2` instead, with a new `client-module-name-collision` **warning**.
