...
- **Operation names that collide with an inherited client member are now renamed instead of breaking the build.** A generated `*Client` extends `HttpClient`, so an operation named `request`, `config`, `buildUrl`, `observe`, `useMiddleware`, or one of the `httpGet`/`httpPost`/… helpers produced a method that shadowed the inherited member with an incompatible signature — and the generated package failed to type-check. Such methods now get an `Operation` suffix (`request` → `requestOperation`, numbered further if a sibling operation already claims that name), accompanied by a new `reserved-client-method-name` **warning** that names the substitution. Only the client method is renamed: the `*Endpoints` object is a plain `as const` that inherits nothing, so its keys — and the endpoint call inside the renamed method — keep the original operation name. The warning is reported once per operation even when both client flavors are emitted, and not at all under `generate-http-client: false`.
...
- **Malformed `Retry-After` headers no longer produce bogus retry delays.** The header was parsed with `parseFloat`, which accepts a numeric _prefix_ and passes through negative and non-finite values — so `"2seconds"` became a 2-second delay, `"-1"` a negative one, and `"Infinity"` a retry that would never fire, hanging the call indefinitely. The complete value must now match a non-negative, finite `delta-seconds` number (integer or decimal, surrounding whitespace tolerated); anything else — an HTTP-date, a trailing unit, `"1e3"`, `NaN` — yields `undefined`, and the client falls back to its own exponential backoff.
